...
resource "aws_iam_server_certificate" "fixture_cert" {
  name_prefix      = "ag-test_cert-${data.aws_caller_identity.current.account_id}-${data.aws_region.current.name}"
...
module "security_group" {
  ...
  version = "1.12.0"
  name    = "ag-alb-sg-https"
  vpc_id  = "${module.vpc.vpc_id}"
  tags    = "${local.tags}"
}

resource "aws_autoscaling_group" "autogrid" {
  name_prefix          = "ag-alb"
  max_size             = 1
  min_size             = 1
...
resource "aws_launch_configuration" "ag-alb" {
  name_prefix   = "autogrid_lc"
...
module "alb" {
  source                   = "./alb_config"
  load_balancer_name       = "ag-test-alb-${random_string.suffix.result}"
